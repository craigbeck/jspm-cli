...
exports.install = function(targets, options) {
  ...
  if (!_targets && Object.keys(targets).length === 1) {
    _targets = targets;
  }
  ...
  .then(function() {
    ...
    // console.log(chalk.bold.yellow("installed"), installed.baseMap, installed.depMap);

    config.loader.emitter.on("config", cfg => {
    ...
    // console.log(chalk.yellow("targets"), chalk.cyan(util.inspect(targets)));
    // console.log();
    // console.log("config.pjson", config.pjson);

    targets = pkg.processDeps(targets, globalConfig.config.defaultRegistry);

    // console.log(chalk.bold.yellow("targets post processDeps ====>\n"), targets);
    ...
    .then(function () {
      ...
      function copyDeps(key, depMap) {
        var deps = _cfg.map[key];
        console.log(chalk.cyan(key), deps);

        if (deps) {
          depMap = Object.assign(Object.assign({}, depMap), {[key]: deps});

          if (typeof deps === "string") {
            // if (depMap.hasOwnProperty(key)) {
            //   console.log(chalk.bold.red("already included"), chalk.cyan(key), depMap);
            //   return depMap;
            // }
            return copyDeps(deps, depMap);
          }
          if (typeof deps === "object") {
            return Object.keys(deps).reduce((acc, dep) => {
              if (depMap[deps[dep]]) {
                // console.log(chalk.red("curcular?"), dep, depMap);
                return depMap;
              }
              return copyDeps(deps[dep], acc);
            }, depMap);
          }
        } else {
          return depMap;
      ...
      var root = Object.keys(_targets)[0];
      depMap = copyDeps(root, depMap);

      console.log(chalk.bold.green("depMap"));
      console.log(chalk.green(JSON.stringify(depMap, null, 2)));
